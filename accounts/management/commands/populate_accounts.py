...
ACCOUNT_DATA = [
    {"name": "Root Account", "acc_type_name": "Root", "parent_name": None},
]
...
def _populate_account(data):
    ...
    data["parent"] = None if parent_name is None else Account.objects.get(name=parent_name)

    return Account.objects.create(**data)


account_populator = TablePopulator(
    _populate_account, lambda x: Account.objects.filter(name=x["name"]).exists(), ACCOUNT_DATA
)
