...
class TestAccountFactory(AccountsModelTestCase):
    def setUp(self):
        super().setUp()
        self.data = {"name": "My Account", "acc_type": AccTypeEnum.BRANCH, "parent": get_root_acc()}
    ...
    def test_invalid_account_type_raises_err(self):
        AccountType.objects.filter(name=self.data["acc_type"].value).update(
...
class TestAccount(AccountsModelTestCase):
    def test_get_acc_type(self):
        acc = AccountTestFactory()
        exp_acc_type = next(x for x in AccTypeEnum if x.value == acc.acc_type.name)
        assert acc.get_acc_type() == exp_acc_type

    def test_get_descendants_ids(self):
        self.populate_accounts()
        root = get_root_acc()
        assert root.get_descendants_ids(True) == [x.id for x in Account.objects.all()]
