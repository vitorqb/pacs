...
from accounts.serializers import AccountSerializer, AccTypeField, JournalSerializer
...
class TestAccountSerializer_creation(PacsTestCase):
    def setUp(self):
        ...
        account_type_populator()
        account_populator()
        self.data = {"name": "Acc", "acc_type": AccTypeEnum.LEAF.value, "parent": get_root_acc().pk}
...
class TestJournalSerializer(PacsTestCase):
    def test_serializes_account_as_pk(self):
        account = Mock(pk=12)
        transaction = Mock(
            pk=1, get_movements_specs=[], get_balance_for_account=Mock(return_value=Balance([]))
        )
    ...
    def test_serializes_initial_balance(self, m_to_representation):
        ...
        assert (
            JournalSerializer(journal).data["initial_balance"] == m_to_representation.return_value
        )
    ...
    def test_serializes_transactions(self, m_to_representation):
        ...
        assert m_to_representation.call_args_list[0] == call(transactions[0])

    @patch.object(BalanceSerializer, "to_representation")
    def test_serializes_balances_from_get_balances(self, m_to_representation):
        m_Journal = Mock()
        ...
        assert m_to_representation.call_args_list[1] == call(m_Journal().get_balances[0])
        assert m_to_representation.call_args_list[2] == call(m_Journal().get_balances[1])
