...
class TestAccountViewset(AccountViewTestCase):
    ...
    def test_get_count_queries(self):
        ...
        lower_accs = [  # noqa
            *AccountTestFactory.create_batch(3, parent=top_accs[0], acc_type=AccTypeEnum.BRANCH),
            *AccountTestFactory.create_batch(2, parent=top_accs[1], acc_type=AccTypeEnum.BRANCH),
        ]
        last_accs = [  # noqa
            *AccountTestFactory.create_batch(3, parent=lower_accs[0], acc_type=AccTypeEnum.LEAF),
            *AccountTestFactory.create_batch(2, parent=lower_accs[2], acc_type=AccTypeEnum.LEAF),
        ]
    ...
    def test_get_for_single_account(self):
        ...
        self.assertEqual(AccountSerializer(acc).data, resp.json())

    def test_post_for_new_account(self):
        self.populate_accounts()
        root_acc = get_root_acc()
        acc_data = {"name": "MyAcc", "acc_type": "Branch", "parent": root_acc.pk}
    ...
    def test_get_journal(self, m_get_all_transactions, m_Journal, m_get_journal_paginator):
        ...
        m_Journal.assert_called_with(account, Balance([]), m_get_all_transactions())
    ...
    def test_get_journal_paginated_first_page(self):
        ...
        assert resp.json()["journal"]["transactions"][0]["pk"] == self.transactions[0].pk

        assert len(resp.json()["journal"]["balances"]) == 1
        movements_specs = self.transactions[0].get_movements_specs()
        assert (
            Decimal(resp.json()["journal"]["balances"][0][0]["quantity"])
    ...
    def test_get_journal_paginated_second_page(self):
        ...
        assert resp.json()["journal"]["transactions"][0]["pk"] == self.transactions[1].pk
        ...
        assert resp.json()["journal"]["balances"][0] == BalanceSerializer(exp_balance).data
